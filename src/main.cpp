...
int main(int argc, char* argv[]) {
    ...
    std::string executable_path = source_path;
    ...
    {
        std::cerr << "Program does not have entry point 'main'" << '\n';
        return 1;
    }
    ...
    if (!target_machine)
    {
        std::cerr << "Failed to create TargetMachine for triple '" << target_triple << "'" << '\n';
        return 1;
    }
    ...
    if (ec)
    {
        std::cerr << "Could not open file '" << object_path << "': " << ec.message() << '\n';
        return 1;
    }
    ...
    if (linkRes != 0)
    {
        std::cerr << "Link command: " << link_cmd << '\n';
        std::cerr << link_out << '\n';
